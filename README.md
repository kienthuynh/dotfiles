...
- **Username**: run `./bootstrap.sh` (it detects your macOS username and offers to set it) OR change the single `user = "kienhuynh"` line in `flake.nix`.
  Everything else (`configuration.nix`, `home.nix`, home directory paths) is threaded from that one variable.
- **Host label** `"mac"`, in three places: `flake.nix` (the `darwinConfigurations."mac"` name), `rebuild.sh:5` (the `#mac` at the end of the flake reference), and `bootstrap.sh`'s first-switch command (also `#mac`).
  All three have to match.
- **CPU architecture**, `hostPlatform` in `configuration.nix` (see Prerequisites above).

**Git identity:** `home.nix` sets my git name and email under `programs.git.settings.user`.
The email is a GitHub `users.noreply` address, so nothing private is published.
If you fork this, change it to your own identity or drop the `user` block, otherwise your commits are authored as me.
